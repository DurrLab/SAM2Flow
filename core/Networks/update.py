...
from .RAFT.gma import Aggregate
from .RAFT.layer import ConvNextBlock
...
# copy from SEA-RAFT + slight modification
class SEAMotionEncoder(nn.Module):
    def __init__(self, args, dim=128):
        super(SEAMotionEncoder, self).__init__()
...
class SEAUpdateBlock(nn.Module):
    def __init__(self, args, hdim=128, cdim=128):
        #net: hdim, inp: cdim
        super(SEAUpdateBlock, self).__init__()
        self.args = args
        self.feature_encoder = SEAMotionEncoder(args, dim=cdim)
        self.feature_pos_enc = PositionEmbeddingSine(num_pos_feats=128, 
                                                     normalize=True, 
                                                     scale=None, 
                                                     temperature=10000)
        self.refine = []
    ...
    def encode_motion_features(self, flow, corr):
        feature = self.feature_encoder(flow, corr)
        pos_enc = self.feature_pos_enc(feature)
